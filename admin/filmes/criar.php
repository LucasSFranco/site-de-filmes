...
<?php 
  ...
  if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "create") {
    ...
    if(count((array)$errorMessages) == 0) {
      try {
        $query = "INSERT INTO filmes (titulo, sinopse, imagem, video) VALUES (?, ?, ?, ?)";
  ...
  <body>
    ...
    <div class="wrapper">
      <form action="?act=create" method="POST" class="form <?php if(isset($errorMessages->form)) { echo 'global-error'; } ?>">
        ...
        <button class="button create" type="submit"> <i class="icon fa fa-plus-square"></i>Criar </button>
